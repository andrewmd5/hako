...
export class CModuleBuilder implements Disposable {
  ...
  constructor(
    context: VMContext,
    name: string,
    initHandler: (initializer: CModuleInitializer) => void
  ) {
    ...
    this.modulePtr = Scope.withScope((scope) => {
      ...
      const modulePtr = hakoExports.HAKO_NewCModule(
        context.pointer,
        moduleName
      );
    ...
    this.vmContext.container.callbacks.registerModuleInitHandler(
      name,
      (initializer) => {
  ...
  setPrivateValue(value: string | boolean | number | object): void {
    ...
    using vmv = this.vmContext.newValue(value);
    this.vmContext.container.exports.HAKO_SetModulePrivateValue(
  ...
  getPrivateValue(): VMValue {
    ...
    return new VMValue(
      this.vmContext,
      this.context.container.exports.HAKO_GetModulePrivateValue(
        this.vmContext.pointer,
        this.modulePtr
      ),
  ...
  get name(): string {
    return Scope.withScope((scope) => {
      ...
      const namePtr = exports.HAKO_GetModuleName(
        this.vmContext.pointer,
        this.modulePtr
      );
      ...
      scope.add(() =>
        this.vmContext.container.memory.freeCString(
          this.vmContext.pointer,
          namePtr
        )
      );
  ...
  addExport(exportName: string): this {
    ...
    Scope.withScope((scope) => {
      ...
      const exportNamePtr = memory.allocateString(
        this.vmContext.pointer,
        exportName
      );
      scope.add(() => memory.freeMemory(this.vmContext.pointer, exportNamePtr));

      const result = exports.HAKO_AddModuleExport(
        this.vmContext.pointer,
        this.modulePtr,
        exportNamePtr
      );
...
export class CModuleInitializer implements Disposable {
  ...
  setExport(exportName: string, value: unknown): void {
    ...
    if (value instanceof VMValue) {
      Scope.withScope((scope) => {
        scope.add(() => value.dispose()); // Add VMValue disposal to scope
        ...
        scope.add(() =>
          memory.freeMemory(this.vmContext.pointer, exportNamePtr)
        );
  ...
  setClass(
    ...
    const classObj = new CModuleClass(
      this.vmContext,
      name,
      constructorFn,
      options
...
export class CModuleClass implements Disposable {
  ...
  constructor(
    ...
    this.classId = Scope.withScope((scope) => {
      const classIdPtr = context.container.memory.allocatePointerArray(
        context.pointer,
        1
      );
      scope.add(() =>
        context.container.memory.freeMemory(context.pointer, classIdPtr)
      );
    ...
    try {
      ...
      const internalConstructor: ClassConstructorHandler = (
        _ctx,
        newTarget,
        args,
        _classId
      ...
      context.container.callbacks.registerClassConstructor(
        this.classId,
        internalConstructor
      );
      if (options.finalizer) {
        context.container.callbacks.registerClassFinalizer(
          this.classId,
          options.finalizer
        );
      }
  ...
  private setupClass(
    ...
    const protoPtr = this.context.container.exports.HAKO_NewObject(
      this.context.pointer
    );
    const protoError = this.context.getLastError(protoPtr);
    if (protoError) {
      throw new HakoError(
        `Prototype creation exception for ${name}`,
        protoError
      );
    }
    ...
    if (options.methods) {
      for (const [methodName, methodFn] of Object.entries(options.methods)) {
        ...
        if (methodError) {
          throw new HakoError(
            `Failed to create method ${methodName}`,
            methodError
          );
        }
    ...
    this.ctorFunction = Scope.withScope((scope) => {
      const namePtr = this.context.container.memory.allocateString(
        this.context.pointer,
        name
      );
      scope.add(() =>
        this.context.container.memory.freeMemory(this.context.pointer, namePtr)
      );
      ...
      if (ctorError) {
        throw new HakoError(
          `Class constructor exception for ${name}`,
          ctorError
        );
      }
    ...
    if (options.staticMethods) {
      for (const [methodName, methodFn] of Object.entries(
        ...
        if (methodError) {
          throw new HakoError(
            `Failed to create static method ${methodName}`,
            methodError
          );
        }
  ...
  getOpaque(instance: VMValue): number {
    this.checkDisposed();
    return this.context.container.exports.HAKO_GetOpaque(
      this.context.pointer,
      instance.getHandle(),
      this.classId
    );
  ...
  [Symbol.dispose](): void {
    ...
    try {
      this.context.container.callbacks.unregisterClassConstructor?.(
        this.classId
      );
      ...
      console.error(
        `Error during CModuleClass ${this.classId} disposal:`,
        error
      );
      this.disposed = true;
    }
